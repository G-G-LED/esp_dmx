...
bool rdm_register_power_state(dmx_port_t dmx_num, rdm_callback_t cb,
  ...
  const rdm_pid_t pid = RDM_PID_POWER_STATE;

  // Attempt to load the value from NVS
  rdm_power_state_t power_state;
  if (!dmx_nvs_get(dmx_num, RDM_SUB_DEVICE_ROOT, pid, &power_state,
                   sizeof(power_state))) {
    power_state = 0;
  }
  ...
  // Allocate parameter data
  if (!dmx_parameter_add(dmx_num, RDM_SUB_DEVICE_ROOT, pid,
                         DMX_PARAMETER_TYPE_NON_VOLATILE, &power_state,
                         sizeof(power_state))) {
    return false;
  }
  ...
  static const rdm_parameter_definition_t definition = {
      ...
      .set = {.handler = rdm_simple_response_handler,
              ...
              .response.format = NULL},
      .pdl_size = sizeof(uint8_t),
      .max_value = RDM_POWER_STATE_NORMAL,
      .min_value = RDM_POWER_STATE_FULL_OFF,
      .units = RDM_UNITS_NONE,
      .prefix = RDM_PREFIX_NONE,
      .description = NULL};
  rdm_definition_set(dmx_num, RDM_SUB_DEVICE_ROOT, pid, &definition);
...
size_t rdm_get_power_state(dmx_port_t dmx_num, rdm_power_state_t *power_state) {
  DMX_CHECK(dmx_num < DMX_NUM_MAX, 0, "dmx_num error");
  DMX_CHECK(power_state != NULL, 0, "power_state is null");
  DMX_CHECK(dmx_driver_is_installed(dmx_num), 0, "driver is not installed");

  return dmx_parameter_copy(dmx_num, RDM_SUB_DEVICE_ROOT, RDM_PID_POWER_STATE,
...
bool rdm_set_power_state(dmx_port_t dmx_num, rdm_power_state_t power_state) {
  ...
  const rdm_pid_t pid = RDM_PID_POWER_STATE;
  if (!dmx_parameter_set(dmx_num, RDM_SUB_DEVICE_ROOT, pid, &power_state,
                         sizeof(power_state))) {
    return false;
  }
  rdm_queue_push(dmx_num, pid);
