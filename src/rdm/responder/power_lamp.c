...
bool rdm_register_lamp_state(dmx_port_t dmx_num, rdm_callback_t cb,
  ...
  const rdm_pid_t pid = RDM_PID_LAMP_STATE;

  // Attempt to load the value from NVS
  rdm_lamp_state_t lamp_state;
  if (!dmx_nvs_get(dmx_num, RDM_SUB_DEVICE_ROOT, pid, &lamp_state,
                   sizeof(lamp_state))) {
    lamp_state = 0;
  }
  ...
  // Allocate parameter data
  if (!dmx_parameter_add(dmx_num, RDM_SUB_DEVICE_ROOT, pid,
                         DMX_PARAMETER_TYPE_NON_VOLATILE, &lamp_state,
                         sizeof(lamp_state))) {
    return false;
  }
  ...
  static const rdm_parameter_definition_t definition = {
      ...
      .max_value = RDM_LAMP_ERROR,
      .min_value = RDM_LAMP_OFF,
      .units = RDM_UNITS_NONE,
      .prefix = RDM_PREFIX_NONE,
      .description = NULL};
  rdm_definition_set(dmx_num, RDM_SUB_DEVICE_ROOT, pid, &definition);
...
size_t rdm_get_lamp_state(dmx_port_t dmx_num, rdm_lamp_state_t *lamp_state) {
  DMX_CHECK(dmx_num < DMX_NUM_MAX, 0, "dmx_num error");
  DMX_CHECK(lamp_state != NULL, 0, "lamp_state is null");
  DMX_CHECK(dmx_driver_is_installed(dmx_num), 0, "driver is not installed");

  return dmx_parameter_copy(dmx_num, RDM_SUB_DEVICE_ROOT, RDM_PID_LAMP_STATE,
                            lamp_state, sizeof(*lamp_state));
}
...
bool rdm_set_lamp_state(dmx_port_t dmx_num, rdm_lamp_state_t lamp_state) {
  ...
  const rdm_pid_t pid = RDM_PID_LAMP_STATE;
  if (!dmx_parameter_set(dmx_num, RDM_SUB_DEVICE_ROOT, pid, &lamp_state,
                         sizeof(lamp_state))) {
    return false;
  }
  rdm_queue_push(dmx_num, pid);
...
bool rdm_register_lamp_on_mode(dmx_port_t dmx_num, rdm_callback_t cb,
  ...
  const rdm_pid_t pid = RDM_PID_LAMP_ON_MODE;

  // Attempt to load the value from NVS
  rdm_lamp_on_mode_t lamp_on_mode;
  if (!dmx_nvs_get(dmx_num, RDM_SUB_DEVICE_ROOT, pid, &lamp_on_mode,
                   sizeof(lamp_on_mode))) {
    lamp_on_mode = 0;
  }
  ...
  // Allocate parameter data
  if (!dmx_parameter_add(dmx_num, RDM_SUB_DEVICE_ROOT, pid,
                         DMX_PARAMETER_TYPE_NON_VOLATILE, &lamp_on_mode,
                         sizeof(lamp_on_mode))) {
    return false;
  }
  ...
  static const rdm_parameter_definition_t definition = {
      ...
      .set = {.handler = rdm_simple_response_handler,
              ...
              .response.format = NULL},
      .pdl_size = sizeof(uint8_t),
      .max_value = RDM_LAMP_ON_MODE_AFTER_CAL,
      .min_value = RDM_LAMP_ON_MODE_OFF,
      .units = RDM_UNITS_NONE,
      .prefix = RDM_PREFIX_NONE,
      .description = NULL};
  rdm_definition_set(dmx_num, RDM_SUB_DEVICE_ROOT, pid, &definition);
...
size_t rdm_get_lamp_on_mode(dmx_port_t dmx_num, rdm_lamp_on_mode_t *lamp_on_mode) {
  DMX_CHECK(dmx_num < DMX_NUM_MAX, 0, "dmx_num error");
  DMX_CHECK(lamp_on_mode != NULL, 0, "lamp_on_mode is null");
  DMX_CHECK(dmx_driver_is_installed(dmx_num), 0, "driver is not installed");

  return dmx_parameter_copy(dmx_num, RDM_SUB_DEVICE_ROOT, RDM_PID_LAMP_ON_MODE,
                            lamp_on_mode, sizeof(*lamp_on_mode));
}
...
bool rdm_set_lamp_on_mode(dmx_port_t dmx_num, rdm_lamp_on_mode_t lamp_on_mode) {
  ...
  const rdm_pid_t pid = RDM_PID_LAMP_ON_MODE;
  if (!dmx_parameter_set(dmx_num, RDM_SUB_DEVICE_ROOT, pid, &lamp_on_mode,
                         sizeof(lamp_on_mode))) {
    return false;
  }
  rdm_queue_push(dmx_num, pid);
...
bool rdm_register_device_power_cycles(dmx_port_t dmx_num, rdm_callback_t cb,
  ...
  const rdm_pid_t pid = RDM_PID_DEVICE_POWER_CYCLES;

  // Attempt to load the value from NVS
  uint32_t device_power_cycles;
  if (!dmx_nvs_get(dmx_num, RDM_SUB_DEVICE_ROOT, pid, &device_power_cycles,
                   sizeof(device_power_cycles))) {
    device_power_cycles = 0;
  }

  // Allocate parameter data
  if (!dmx_parameter_add(dmx_num, RDM_SUB_DEVICE_ROOT, pid,
                         DMX_PARAMETER_TYPE_NON_VOLATILE, &device_power_cycles,
                         sizeof(device_power_cycles))) {
    return false;
  }
  ...
  // Define the parameter
  static const rdm_parameter_definition_t definition = {
#ifdef CONFIG_RDM_PID_DEVICE_POWER_CYCLES_DISABLE_SET
      .pid_cc = RDM_CC_GET,
#else
      .pid_cc = RDM_CC_GET_SET,
#endif
...
size_t rdm_get_device_power_cycles(dmx_port_t dmx_num, uint32_t *device_power_cycles) {
  DMX_CHECK(dmx_num < DMX_NUM_MAX, 0, "dmx_num error");
  DMX_CHECK(device_power_cycles != NULL, 0, "device_power_cycles is null");
  DMX_CHECK(dmx_driver_is_installed(dmx_num), 0, "driver is not installed");

  return dmx_parameter_copy(dmx_num, RDM_SUB_DEVICE_ROOT, RDM_PID_DEVICE_POWER_CYCLES,
                            device_power_cycles, sizeof(*device_power_cycles));
}
...
bool rdm_set_device_power_cycles(dmx_port_t dmx_num, uint32_t device_power_cycles) {
  ...
  const rdm_pid_t pid = RDM_PID_DEVICE_POWER_CYCLES;
  if (!dmx_parameter_set(dmx_num, RDM_SUB_DEVICE_ROOT, pid, &device_power_cycles,
                         sizeof(device_power_cycles))) {
    return false;
  }
  rdm_queue_push(dmx_num, pid);
