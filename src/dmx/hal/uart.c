// RCD added module_lookup to fix calls to periph_module_enable et al.
    ...
    if (intr_flags & DMX_INTR_RX_ALL) {
      ...
      while (err == DMX_OK) {
        if (rdm_type == RDM_TYPE_IS_DISCOVERY) {
          ...
          } else if (!rdm_read_header(dmx_num, NULL, NULL)) {
            rdm_type = RDM_TYPE_IS_NOT_RDM;
            continue;  // Packet is malformed - treat it as DMX
          } else {
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 2, 0)
const periph_module_t module_lookup[3] = {
  PERIPH_UART0_MODULE,
  PERIPH_UART1_MODULE,
  PERIPH_UART2_MODULE,
};
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 2, 0)
  periph_module_enable(module_lookup[dmx_num]);
#else
...
#else
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 2, 0)
    periph_module_reset(module_lookup[dmx_num]);
#else
    periph_module_reset(uart_periph_signal[dmx_num].module);
#endif
#endif
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 2, 0)
    periph_module_disable(module_lookup[uart->num]);
#else
